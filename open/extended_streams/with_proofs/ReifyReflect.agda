...
mutual
  ...
  reify < σ > (neSV x) = ne x
  reify < σ > (streamSV h t) = nstream (reify σ h) (∞reify t )

  ∞reify : ∀{Γ σ} → ∞StreamVal (Val Γ σ) (Ne Γ < σ >) → ∞Nf<> Γ σ
  ...
  renvalReflect {Γ} {Δ} {< σ >} ρ l = proof neSV (renNe ρ l) ≡⟨⟩ neSV (renNe ρ l) ∎
  ...
  reifyRenvalSV ρ (streamSV h t) = nstream∼ (≅toNf∼ (reifyRenval ρ h)) (∞reifyRenvalSV ρ t)

  ∞reifyRenvalSV : ∀{Γ Δ σ i}(ρ : Ren Γ Δ)(s : ∞StreamVal (Val Γ σ) (Ne Γ < σ >)) → renNf∞ ρ (∞reify s) ∞Nf⟨ i ⟩∼  ∞reify (∞renvalSV {σ = σ} ρ s)
  ∼nforce (∞reifyRenvalSV ρ s) = reifyRenvalSV ρ (vforce s)
