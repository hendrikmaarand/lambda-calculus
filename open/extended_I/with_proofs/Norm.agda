...
renvaleval {σ = σ ⇒ τ} γ ρ (lam t) = Σeq 
  (iext λ Δ' → ext λ (α : Ren _ _) → ext λ v → cong (λ (f : Env _ _) → eval (f << v) t) (iext λ σ' → ext λ x → renvalcomp {σ = σ'} ρ α (γ x))) 
  refl 
  (iext λ Δ → iext λ Δ' → ext λ (ρ₁ : Ren _ _) → ext λ (ρ₂ : Ren _ _) → ext λ v → fixedtypesleft (cong (λ (f : Env _ _) → 
                   renval {σ = τ} ρ₂ (eval (f << v) t)) (iext λ σ' → ext λ x → renvalcomp {σ = σ'} ρ ρ₁ (γ x))))
...
renvaleval {σ = σ} γ ρ (rec z f n) = proof
  ...
  ≅⟨ cong₃ (nfold {σ = σ}) (renvaleval γ ρ z) (renvaleval γ ρ f) (renvaleval γ ρ n) ⟩
   nfold {σ = σ} (renval {σ = σ} ρ (eval γ z)) 
         ...
         (renNf ρ (eval γ n))
  ≅⟨ sym (renvalnfold {σ = σ} ρ (eval γ z) (eval γ f) (eval γ n)) ⟩
